...
console.log(product.cantidad)
  ...
  const submitForm = async(formData,clear) => {
    ...
    clear()
    setTimeout(()=> {setPopup(false), nav.navigate("ProductsScreen") },1000)
      }
  return (
    <Formik
      initialValues={{name: product.nombre, descripcion: product.descripcion, precioCompra: String(product.precioCompra), precioVenta: String(product.precioVenta), cantidad: String(product.cantidad) , categoria: product.categoria }}
    ...
    >
      {({
        ...
        >
          ...
          <Text style={[styles.label,{color:colors.text}]}>Precio de Costo</Text>
          <TextInput
          ...
          <Text style={[styles.label,{color:colors.text}]}>Precio de Venta</Text>
          <TextInput
            keyboardType={"numeric"}
            style={[styles.textInput,{color:colors.text,backgroundColor:colors.card}]}
          ...
          <PopUp visibility={popup} message="¡Se guardaron los cambios con éxito!" ></PopUp>
        </ScrollView>
      )}
    </Formik>
  );
